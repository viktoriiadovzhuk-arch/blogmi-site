...
import { PROMO_DISCOUNT } from "./_pricing.mjs";
...
// Нормалізуємо UTM у плоскі рядкові поля (як у webhook flattenUtm)
function flattenUtm(utm) {
  const out = {};
  if (utm && typeof utm === "object") {
    for (const k of ["utm_source","utm_medium","utm_campaign","utm_content","utm_term"]) {
      if (utm[k] != null) out[k] = String(utm[k]).slice(0, 200);
    }
  }
  return out;
}
...
export default async (req) => {
  ...
  try {
    ...
    const utmFlat = flattenUtm(utm);

    console.log(`🎓 register-lesson called for email=${emailKey}, utm=${JSON.stringify(utmFlat)}`);
    ...
    // ===== ПЕРЕВІРКА ІСНУЮЧОГО ("1 email = 1 подія в SP назавжди") =====
    ...
    // Універсальний код −1000 на будь-який тариф (drabina рахує ціну на момент оплати)
    const promoData = {
      code,
      email: emailKey,
      type: "minus",
    ...
    try {
      ...
      const resp = await fetch(SP_LESSON_EVENT, {
        ...
        body: JSON.stringify({
          ...
          discount: String(PROMO_DISCOUNT),
          ...utmFlat
        }),
